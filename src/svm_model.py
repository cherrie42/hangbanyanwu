...
# 绘制学习曲线
print("绘制学习曲线...")
plot_learning_curve(best_svm, X_train_scaled, y_train, "SVM学习曲线")

# 在验证集上评估
val_pred = best_svm.predict(X_val_scaled)
val_pred_proba = best_svm.predict_proba(X_val_scaled)
f_score = fbeta_score(y_val, val_pred, beta=0.5)
acc = accuracy_score(y_val, val_pred)
...
# 绘制决策边界（使用前两个特征）
print("绘制决策边界...")
plot_decision_regions(X_val_scaled[:, :2], y_val, best_svm, "SVM决策边界可视化")
...
# 8. 保存模型
print("\n8. 保存模型...")
joblib.dump(best_svm, './model/flight_delay_svm_model.pkl')
print("模型已保存为 ./model/flight_delay_svm_model.pkl")